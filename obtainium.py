...
def main():
    ...
    for section_name, values in data.items():
        # Skip top-level scalar values (int, str, bool, list, etc.)
        # Only process dictionary items (TOML tables)
        if not isinstance(values, dict):
            # Optional: print a warning if you want to debug
            # print(f"Warning: skipping top-level key '{section_name}' (value is {type(values).__name__})")
            continue
